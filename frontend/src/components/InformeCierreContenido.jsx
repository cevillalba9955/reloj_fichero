...
// 018-informe-cierre-periodo — Contenido del informe de cierre (sin modal ni
// botones): sello de emisión + Resumen por empleado + Detalle día por día +
// Pendientes. Es lo que se muestra al "Ver informe" (dentro de un Dialogo) y lo
// que se imprime al "Descargar PDF" (dentro de un iframe aislado). Componente
// de presentación puro: recibe la `vista` ya resuelta (Principio I).
...
function marcasDia(d) {
  const marcas = [];
  if (d.corregida) marcas.push(d.motivoCorreccion ? `${d.motivoCorreccion}` : 'corregida');
...
export function imprimirEnIframe(titulo, htmlContenido) {
  ...
  doc.write(
    `<!doctype html><html lang="es"><head><meta charset="utf-8"><title>${titulo}</title>` +
      `<style>body{font-family:system-ui,-apple-system,'Segoe UI',Roboto,sans-serif;margin:0}${ESTILOS_INFORME}</style>` +
...
const InformeCierreContenido = forwardRef(function InformeCierreContenido({ vista }, ref) {
  const { sello, resumen, detalle, pendientes, obsoleto } = vista;
  const fechaEmision = new Date(sello.emitidoEn).toLocaleString('es-AR');

  return (
    <div className="informe-cierre-contenido" ref={ref}>
...
export default InformeCierreContenido;
