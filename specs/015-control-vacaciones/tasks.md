...
- [X] T017 [P] [US1] Test de integración en `tests/integration/vacaciones.integration.test.js` (arrancar este archivo, se completa en US4): asignar vacaciones con un rango que cruza de un período a otro (Acceptance Scenario US1.3) → verificar que ambos `P<periodo>/calendario.json` tienen la Justificación-espejo `motivoId: 'vacaciones-anual'`/`tipoPago: 'No paga'` en los días que corresponden a cada uno; que `resumen-presentismo.js` no acredita jornada esperada esos días (sin cambios de código); y que `resumen-periodo.js` los computa en el **contador propio `vacaciones`**, EXCLUIDOS de `ausencias` y de `licencia` (clarificación spec 2026-09-08, Acceptance Scenario US1.6, FR-006; ver T045 para el cambio en `resumen-periodo.js`/`domain/jornada.js`).
...
- [X] T045 [P] Señalar en el calendario/resumen del legajo las fichadas nuevas que lleguen para un día marcado `Vacaciones` (FR-017). Se descubrió durante US1 (T017) que `aplicarAjustes` (`src/presentismo/domain/jornada.js`) sólo aplicaba la Justificación sobre días `Laborable`, descartando en silencio la marca `Vacaciones` en días `No Laborable`/`Feriado`; se generalizó para cualquier clasificación (ver también T-jornada tests) y se agregó un test de integración dedicado (`fichadas sobre un día No Laborable marcado Vacaciones señalan revisión, no lo descartan (FR-017)`, `tests/integration/vacaciones.integration.test.js`). Además, en `src/presentismo/domain/resumen-periodo.js` se agregó el contador propio `vacaciones` (días con `motivoId === 'vacaciones-anual'`), EXCLUIDO de `ausencias` (condición `&& !esVacaciones`) y de `licencia` (clarificación spec 2026-09-08, FR-006; contrato en `contracts/web-api.md` → "Efecto sobre `GET /api/resumen-periodo`").
  - **Fix de producción (legajo 9, 7-8/09/2026)**: el branch "llegaron fichadas" de `aplicarAjustes` conservaba las horas reales y el estado `Completa`/`Incompleta` para CUALQUIER `tipoPago`, así que un día `Laborable` de Vacaciones con fichadas válidas sumaba horas trabajadas y una jornada completa además de marcar la columna `vacaciones`. Corregido: con `tipoPago: 'No paga'` (incluida la Justificación-espejo de Vacaciones) las fichadas nunca acreditan — el día pasa a `Sin fichadas` con 0 horas y solo se señala `requiereJustificacionRevision`; `Paga` mantiene el comportamiento previo (las horas reales prevalecen). Tests: `presentismo-jornada.test.js` ("fichadas que llegan después de justificar (No paga / Vacaciones) NO acreditan horas ni jornada") y `vacaciones.integration.test.js` ("fichadas sobre un día Laborable marcado Vacaciones no suman horas ni presentismo (bug 7-8/09)").
