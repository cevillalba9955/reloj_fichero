...
- [X] T020 [P] [US2] Frontend test in `frontend/src/components/AccionInformeCierre.test.jsx`: with `cerrado={false}` and `anticipadoQ1Disponible={false}`, only the existing plain note renders — no "Emitir informe de la primera quincena" button, no alert.
...
- [X] T030 [P] Update `docs/presentismo.md`: document that in QUINCENAL mode the primera-quincena (`Q1`) report can be emitted manually from "Resumen del Período" once day 15 has passed and before the month is closed (`POST …/informe-cierre?tramo=Q1` on an open period), that such copies carry `sello.anticipado: true`, and that closing the month replaces them with the definitive closing copy.
- [X] T031 [P] Run the full backend suite `node --test` and the full frontend suite `cd frontend; npx vitest run`; fix any regression in feature-018 / feature-021 tests caused by the gate rework or the extra `sello.anticipado` field.
- [X] T032 Execute `specs/022-informe-primera-quincena-anticipado/quickstart.md` §1 (automated) and §2 (API by curl, QUINCENAL) and confirm every listed expectation.
- [~] T033 Manual UI pass per `quickstart.md` §3. Partially done via a live seeded stack (`npm run web` QUINCENAL + `PRESENTISMO_HOY=2026-09-20`, `npm run dev`): the app loads, "Resumen del Período" on **Q2** correctly shows only the plain note (no anticipated action), and end-to-end through the vite `/api` proxy `GET /api/resumen-periodo?periodo=<PER>-Q1` returns `emisionAnticipadaQ1Disponible: true` (Q2 → `false`) and `POST …/informe-cierre?tramo=Q1` on the open period returns `sello.anticipado: true`, `modo: 'manual'`, range days 1–15. The Q1-selected click-through (select Q1 → "Emitir…" → modal) could not be driven because the automated browser window was hidden and the antd Select would not open; that exact wiring is covered by `PaginaResumenPeriodo.test.jsx` / `AccionInformeCierre.test.jsx`.
